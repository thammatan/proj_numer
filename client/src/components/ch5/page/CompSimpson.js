...
const CompSimpson = (props) => {
  ...
  const show = () => {
    api.getCh5().then(res => {
      ...
      setdata(cals.simpson(res.data.data[1].fx, res.data.data[1].a, res.data.data[1].b, 2))
    });
  }
  const ok = () => {
    if ((fn !== null) && props.reg.test(fn) && !(/[ex][ex]+/i).test(fn)&& !(a===b)) {
    setdata(cals.simpson(fn, a, b, n))
    }else {
