...
const CompTrape = (props) => {
  ...
  const show = () => {
    api.getCh5().then(res => {
      ...
      setdata(cals.trapezoidal(res.data.data[0].fx, res.data.data[0].a, res.data.data[0].b, 2))
    });
  }
  const ok = () => {
    
    if ((fn !== null) && props.reg.test(fn) && !(/[ex][ex]+/i).test(fn)&&!(a===b)) {
    setdata(cals.trapezoidal(fn, a, b, n))
    }else {
